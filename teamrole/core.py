...
class TeamRole(commands.Cog):  
    ...
    async def setup(self, ctx):  
        ...
        try:  
            ...
            # Ensure the team role is placed below the bot's top role  
            if bot_top_role:  
                # Find the highest possible position below the bot's top role  
                target_position = bot_top_role.position - 1  
                # Ensure the position is valid (greater than 0)  
            ...
            try:  
                ...
                if bot_top:  
                    ...
                    if role.position >= bot_top.position:  
                        try:  
                            # Place it just below the bot's top role  
                            new_position = bot_top.position - 1  
                            new_position = max(new_position, 1)  # Ensure it's at least 1  
