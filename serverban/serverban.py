...
class ServerBan(red_commands.Cog):
        ...
        try:
            ...
            ban_embed = discord.Embed(
                ...
                description=(f"**Reason:** {reason}\n\n**Servers:** "
                             f"{'KCN Globalban' if is_global_flag else interaction.guild.name}\n\n"
    ...
    async def global_ban_user(self, interaction: discord.Interaction, target_user: discord.Member):
        """
        A user context menu command to globally ban a user by right-clicking on them.
        """
        # Check if the user has permissions to perform a global ban
        if interaction.user.id not in self.ALLOWED_GLOBAL_IDS:
            return await interaction.response.send_message(
                embed=self._error_embed("You are not authorized to perform this action."),
                ephemeral=True
            )
        ...
        # Notify the target user about the ban
        try:
            ban_embed = discord.Embed(
                title="You have been banned",
                description=(
                    f"**Reason:** {reason}\n\n**Servers:** KCN Globalban\n\n"
                    "You may appeal using the provided link. Appeals will be reviewed within 12 hours."
                ),
                color=discord.Color.red()
            )
            ban_embed.add_field(name="Appeal Link", value=self.APPEAL_LINK, inline=False)
            ban_embed.set_thumbnail(url=self.BAN_GIF)
            await target_user.send(embed=ban_embed)
        except discord.HTTPException:
            pass  # Ignore if the user cannot be DMed

        # Perform the global ban
        guilds = [g for g in self.bot.guilds if g.id not in self.server_blacklist]
        results = []
        for guild in guilds:
            try:
                is_banned = False
                async for entry in guild.bans():
                    if entry.user.id == target_user.id:
                        is_banned = True
                        break
                if not is_banned:
                    await guild.ban(discord.Object(id=target_user.id), reason=reason)
                    results.append(f"✅ {guild.name}")
                else:
                    results.append(f"⚠️ {guild.name}: Already banned")
            except Exception as e:
        ...
        # Log the global ban
        await self.log_global_ban(target_user, interaction.user, reason)

        # Respond back to the moderator
        await interaction.followup.send(
            embed=discord.Embed(
                title="Global Ban Results",
                description="\n".join(results),
                color=discord.Color.orange()
            ),
            ephemeral=True
