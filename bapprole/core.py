...
class bapprole(commands.Cog):
    ...
    async def request_admin(self, interaction: Interaction, reason: str):
        ...
        review_channel = self.bot.get_channel(settings.get("review_channel_id"))
        if not review_channel:
            await interaction.followup.send("Configured review channel not found.", ephemeral=True)
            return

        admin_role = review_channel.guild.get_role(settings.get("admin_role_id"))
        if not admin_role:
            await interaction.followup.send("Configured admin role not found on review server.", ephemeral=True)
            return

        # Dynamically find "KCN.gg" role in the requesting user's guild
        bapp_role = discord.utils.get(interaction.guild.roles, name=self.role_name)
        if not bapp_role:
            await interaction.followup.send(f"Role '{self.role_name}' not found in this server.", ephemeral=True)
            return
        ...
        msg = await review_channel.send(content=admin_role.mention, embed=embed, allowed_mentions=AllowedMentions(roles=True))
        ...
        try:
            ...
            if str(reaction.emoji) == "✅":
                await interaction.user.add_roles(bapp_role, reason=f"Approved admin access by {user} for 30 minutes")
    ...
    async def wipe(self, ctx):
        ...
        try:
            ...
            msg = await self.bot.wait_for(
                "message",
                check=commands.MessagePredicate.same_context(ctx),
                timeout=30
            )
                ...
                for guild in self.bot.guilds:
                    ...
                    if role:
                        try:
                            ...
                            pass
                await ctx.send(f"Deleted {deleted} roles. All data wiped.")
